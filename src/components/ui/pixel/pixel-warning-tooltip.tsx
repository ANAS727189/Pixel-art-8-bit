...
const warningTooltipVariants = cva(
  "pixel-borders border-4 p-3 text-sm font-medium shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] z-50 overflow-hidden font-[family-name:var(--font-press-start)] animate-in fade-in-0 zoom-in-95 duration-0",
  {
    variants: {
      severity: {
        warning: "border-[#ffa500] bg-[#fff4e6] text-[#ff8c00] dark:bg-[#332200] dark:text-[#ffa500]",
        error: "border-[#ff0000] bg-[#ffe6e6] text-[#cc0000] dark:bg-[#330000] dark:text-[#ff4444]",
        info: "border-[#0099ff] bg-[#e6f7ff] text-[#0077cc] dark:bg-[#001a33] dark:text-[#66ccff]",
        critical: "border-[#ff00ff] bg-[#ffe6ff] text-[#cc00cc] dark:bg-[#330033] dark:text-[#ff66ff]",
      },
      size: {
...
>(({ className, children, showIcon = true, iconSize = 16, severity = "warning", ...props }, ref) => {
  const iconColors = {
    warning: "text-[#ffa500]",
    error: "text-[#ff0000]",
    info: "text-[#0099ff]",
  ...
  return (
    ...
    >
      {showIcon && (
        <AlertTriangle className={cn("animate-pulse", iconColors[severity])} size={iconSize} />
      )}
      {children}
    </TooltipPrimitive.Trigger>
  );
...
>(({ className, sideOffset = 4, severity, size, showWarningIcon = true, children, ...props }, ref) => (
