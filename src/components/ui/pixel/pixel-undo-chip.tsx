...
import { X, Undo2, CheckCircle2 } from "lucide-react";
...
const undoChipVariants = cva(
  "pixel-borders border-4 border-black p-3 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] font-[family-name:var(--font-press-start)] text-xs flex items-center gap-3 animate-in slide-in-from-bottom-5 duration-0",
  {
    variants: {
      variant: {
        default: "bg-[#4ade80] text-black dark:bg-[#15803d] dark:text-white dark:border-[#22c55e]",
        info: "bg-[#60a5fa] text-black dark:bg-[#1e40af] dark:text-white dark:border-[#3b82f6]",
        warning: "bg-[#fbbf24] text-black dark:bg-[#92400e] dark:text-white dark:border-[#f59e0b]",
        error: "bg-[#f87171] text-black dark:bg-[#991b1b] dark:text-white dark:border-[#ef4444]",
      },
      size: {
...
export function PixelUndoChip({
  ...
  return (
    <div className={cn(undoChipVariants({ variant, size }), className)}>
      {showIcon && <CheckCircle2 className="shrink-0" size={size === "sm" ? 12 : size === "lg" ? 18 : 14} />}
      ...
      {onUndo && (
        ...
        >
          {showUndoIcon && <Undo2 size={size === "sm" ? 8 : size === "lg" ? 12 : 10} />}
          Undo
        </button>
      )}
...
const UndoChipContext = React.createContext<UndoChipContextValue | undefined>(undefined);
...
export function PixelUndoChipProvider({
  ...
  const [chips, setChips] = React.useState<Array<{
    id: string;
    message: string;
    onUndo?: () => void;
    duration?: number;
...
export function usePixelUndoChip() {
  ...
  if (!context) {
    throw new Error("usePixelUndoChip must be used within PixelUndoChipProvider");
  }
  return context;
}
