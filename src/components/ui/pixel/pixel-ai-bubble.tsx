...
const aiBubbleVariants = cva(
  "pixel-borders border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] font-[family-name:var(--font-press-start)] inline-flex items-center gap-3 animate-in fade-in-0 zoom-in-95 duration-0",
  {
    variants: {
      variant: {
        thinking: "bg-[#e0e7ff] text-[#4338ca] dark:bg-[#1e1b4b] dark:text-[#818cf8] dark:border-[#6366f1]",
        ready: "bg-[#d1fae5] text-[#065f46] dark:bg-[#064e3b] dark:text-[#6ee7b7] dark:border-[#10b981]",
        processing: "bg-[#fef3c7] text-[#92400e] dark:bg-[#451a03] dark:text-[#fcd34d] dark:border-[#f59e0b]",
        error: "bg-[#fee2e2] text-[#991b1b] dark:bg-[#450a0a] dark:text-[#fca5a5] dark:border-[#ef4444]",
        typing: "bg-[#ddd6fe] text-[#5b21b6] dark:bg-[#2e1065] dark:text-[#c4b5fd] dark:border-[#8b5cf6]",
      },
      size: {
...
const PixelAIBubble = React.forwardRef<HTMLDivElement, PixelAIBubbleProps>(
  ({ className, variant = "thinking", size = "md", message, showIcon = true, animate = true, ...props }, ref) => {
    const getIcon = () => {
      ...
      switch (variant) {
        ...
        case "processing":
          return <Sparkles className={cn("shrink-0", animate && "animate-pulse")} size={iconSize} />;
        case "error":
          return <AlertCircle className="shrink-0" size={iconSize} />;
        case "typing":
          return <Bot className={cn("shrink-0", animate && "animate-pulse")} size={iconSize} />;
        default:
